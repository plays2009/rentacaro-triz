...
<div class="w3-modal-content w3-card-4" style="padding-left: 12px">
  ...
  ?>
    <div class="w3-dropdown-hover w3-display-container" style="background-color: #ffffff">
      ...
      <div class="w3-dropdown-content w3-display-middle" style="background-color: rgba(0, 0, 0, 0)">
        <button onclick="window.open('editMidImage.php?idx=<?=$getImageData[idx]?>', '_blank', 'width=600 height=400 left=500 top=400')" class="w3-button w3-round-large w3-blue" style="margin: 4px; font-size: 16px;">수정</button>
        <button onclick="location.href='deleteMidImage.php?idx=<?=$getImageData[idx]?>'" class="w3-button w3-round-large w3-blue" style="margin: 4px; font-size: 16px;">삭제</button>
      </div>
    </div>
    <?php } else { // 이미지가 데이터베이스에 등록이 되어 있지 않을 시 ?>
      <div class="w3-dropdown-hover w3-display-container" style="background-color: #ffffff">
        ...
        <div class="w3-dropdown-content w3-display-middle" style="background-color: rgba(0, 0, 0, 0)">
          <button onclick="window.open('editMidImage.php?idx=<?=$getImageData[idx]?>', '_blank', 'width=600 height=200 left=500 top=400')" class="w3-button w3-round-large w3-blue" style="margin: 4px; font-size: 16px;">수정</button>
          <button onclick="location.href='deleteMidImage.php?idx=<?=$getImageData[idx]?>'" class="w3-button w3-round-large w3-blue" style="margin: 4px; font-size: 16px;">삭제</button>
        </div>
      </div>
  <?php }  } ?>
  ...
  <footer class="w3-container">
    <h5>이미지를 수정하거나 삭제하려면 해당 이미지를 클릭하시고, 추가하려면 <a onclick="window.open('addMidImage.php', '_blank', 'width=600 height=400 left=500 top=400')" style="color: blue; cursor: pointer"><i>여기</i></a>를 눌러주세요</h5>
  </footer>
</div>
